...
async function initUser() {
  try {
    const res = await fetch(`https://folio2resume.onrender.com/api/auth/user/${userId}`);
    if (res.ok) {
...
if (activeResumeId) {
  fetch(`https://folio2resume.onrender.com/api/resume/${activeResumeId}`)
    .then(async res => {
  ...
  fetch(`https://folio2resume.onrender.com/api/portfolio/user/${userId}`)
    .then(res => res.json())
    .then(portfolios => {
...
async function generateResume() {
  ...
  try {
    ...
    if (!p) {
      const res = await fetch(`https://folio2resume.onrender.com/api/portfolio/user/${userId}`);
...
async function saveResume() {
  ...
  try {
    const res = await fetch("https://folio2resume.onrender.com/api/resume/save", {
      method: "POST",
